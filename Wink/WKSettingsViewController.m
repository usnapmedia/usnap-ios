...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *doneBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Done", @"").uppercaseString style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonTouched:)];
    ...
    [contactAttrString addAttribute:NSKernAttributeName value:[NSNumber numberWithInteger:2] range:[contactAttrString.string rangeOfString:contactAttrString.string]];
    [contactAttrString addAttribute:NSForegroundColorAttributeName value:[UIColor whiteColor] range:[contactAttrString.string rangeOfString:contactAttrString.string]];
    ...
    [signoutAttrString addAttribute:NSKernAttributeName value:[NSNumber numberWithInteger:2] range:[signoutAttrString.string rangeOfString:signoutAttrString.string]];
    [signoutAttrString addAttribute:NSForegroundColorAttributeName value:self.signOutButton.titleLabel.textColor range:[signoutAttrString.string rangeOfString:signoutAttrString.string]];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [WKUser  currentUser].socialNetworks.count;
}
...
- (IBAction)contactButtonTouched:(id)sender {
    [UIAlertView showWithTitle:[WKUser currentUser].manager.phone message:nil cancelButtonTitle:NSLocalizedString(@"Cancel", @"") otherButtonTitles:@[NSLocalizedString(@"Call", @"")] tapBlock:^(UIAlertView *alertView, NSInteger buttonIndex) {
        if (buttonIndex > 0) {
...
- (IBAction)signOutButtonTouched:(id)sender {
    [UIAlertView showWithTitle:NSLocalizedString(@"Sign Out", @"") message:NSLocalizedString(@"Are you sure you would like to sign out of Wink?", @"") cancelButtonTitle:NSLocalizedString(@"Cancel", @"") otherButtonTitles:@[NSLocalizedString(@"Sign Out", @"")] tapBlock:^(UIAlertView *alertView, NSInteger buttonIndex) {
        if (buttonIndex > 0) {

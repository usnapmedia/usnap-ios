...
- (void)viewDidAppear:(BOOL)animated {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [UIView animateWithDuration:duration delay:(delay * 0.0f) usingSpringWithDamping:damping initialSpringVelocity:velocity options:UIViewAnimationOptionCurveLinear animations:^{
            self.usernameTextfield.transform = CGAffineTransformIdentity;
            self.usernameTextfield.alpha = 1.0f;
        } completion:nil];
        
        [UIView animateWithDuration:duration delay:(delay * 1.0f) usingSpringWithDamping:damping initialSpringVelocity:velocity options:UIViewAnimationOptionCurveLinear animations:^{
            self.passwordTextfield.transform = CGAffineTransformIdentity;
            self.passwordTextfield.alpha = 1.0f;
        } completion:nil];
        
        [UIView animateWithDuration:duration delay:(delay * 2.0f) usingSpringWithDamping:damping initialSpringVelocity:velocity options:UIViewAnimationOptionCurveLinear animations:^{
...
- (IBAction)loginButtonTouched:(id)sender {
    ...
    if (self.usernameTextfield.text.length > 0 && self.passwordTextfield.text.length > 0) {
        ...
        UIAlertView *alert = [UIAlertView showWithTitle:NSLocalizedString(@"Signing In...", @"") message:nil cancelButtonTitle:nil otherButtonTitles:nil tapBlock:nil];
        ...
        [WKWinkConnect winkConnectLoginWithUsername:self.usernameTextfield.text password:self.passwordTextfield.text success:^(id response) {
            ...
            [UIAlertView showWithTitle:NSLocalizedString(@"Error", @"") message:NSLocalizedString(@"Please verify your username and password are correct and try again!", @"") cancelButtonTitle:NSLocalizedString(@"ok", @"") otherButtonTitles:nil tapBlock:nil];
        }];
    }
    else {
        [UIAlertView showWithTitle:NSLocalizedString(@"Missing Information", @"") message:NSLocalizedString(@"Both username and password are required to login", @"") cancelButtonTitle:NSLocalizedString(@"ok", @"") otherButtonTitles:nil tapBlock:nil];
    }
}
